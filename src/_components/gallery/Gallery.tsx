...
const Gallery: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setColumns({
        column1: data.media.filter((_, index) => index % 3 === 0),
        column2: data.media.filter((_, index) => index % 3 === 1),
        column3: data.media.filter((_, index) => index % 3 === 2),
      });
  ...
  return (
    <div className={style.gallery_container}>
      <div className={style.gallery_column}>
        {columns.column1.map((src, index) => (
          <GalleryImage image={src} key={index} />
        ))}
      </div>
      <div className={style.gallery_column}>
        {columns.column2.map((src, index) => (
          <GalleryImage image={src} key={index} />
        ))}
      </div>
      <div className={style.gallery_column}>
        {columns.column3.map((src, index) => (
          <GalleryImage image={src} key={index} />
        ))}
      </div>
    </div>
